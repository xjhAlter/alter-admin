        ...
        function (Animation, SimpleTimeline, TweenLite) {
            ...
            p.totalDuration = function (value) {
                ...
                return this._repeat === -1
                    ? this
                    : this.duration((value - this._repeat * this._repeatDelay) / (this._repeat + 1))
            }
    ...
    _gsScope._gsDefine(
		...
		*/
            ...
            p.set = function (target, vars, position) {
                return this.add(
                    new TweenLite(target, 0, _defaultImmediateRender(this, vars, null, true)),
                    position
                )
            }
            ...
            p.seek = function (position, suppressEvents) {
                return this.totalTime(
                    typeof position === 'number' ? position : this._parseTimeOrLabel(position),
            ...
            p.render = function (time, suppressEvents, force) {
                ...
                if (time >= totalDur - _tinyNum && time >= 0) {
                    ...
                    self._rawPrevTime =
                        self._duration || !suppressEvents || time || self._rawPrevTime === time
                            ? time
        ...
        function (TweenPlugin, TweenLite) {
            ...
            var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
                ...
                _calculateOffset = (_internals.calculateOffset = function (t, p, cs) {
                    ...
                    return (
                        t['offset' + dim] -
                        (_convertToPixels(t, p, parseFloat(v), v.replace(_suffixExp, '')) || 0)
                    )
                }),
			...
			*/
            ...
            _registerComplexSpecialProp('border', {
                ...
                formatter: function (v) {
                    var a = v.split(' ')
                    return (
                        a[0] + ' ' + (a[1] || 'solid') + ' ' + (v.match(_colorExp) || ['#000'])[0]
                    )
                },
            })
            ...
            _registerComplexSpecialProp('className', {
                parser: function (t, e, p, cssp, pt, plugin, vars) {
                    ...
                    pt.e =
                        e.charAt(1) !== '='
                            ? e
                            : b.replace(new RegExp('(?:\\s|^)' + e.substr(2) + '(?![\\w-])'), '') +
        ...
        function (Ease) {
            ...
            _wrap(
                ...
                _createElastic(
                    ...
                    function (p) {
                        return (
                            this._p1 * Math.pow(2, -10 * p) * Math.sin((p - this._p3) * this._p2) +
                            1
                        )
                    },
                    0.3
...
;(function (window, moduleName) {
    ...
    var _namespace = function (ns) {
            ...
            return function (obj) {
                return (
                    ...
                    (obj instanceof Array ||
                        (typeof obj === 'object' && !!obj.push && toString.call(obj) === array))
                )
            }
        })(),
        a,
        ...
        Definition = function (ns, dependencies, func, global) {
            ...
            this.check = function (init) {
                ...
                while (--i > -1) {
                    if (
                        (cur = _defLookup[dependencies[i]] || new Definition(dependencies[i], []))
                            .gsClass
                    ) {
    ...
    while (--i > -1 && !_reqAnimFrame) {
        ...
        _cancelAnimFrame =
            window[a[i] + 'CancelAnimationFrame'] || window[a[i] + 'CancelRequestAnimationFrame']
    }
    ...
    p.endTime = function (includeRepeats) {
        return (
            this._startTime +
            (includeRepeats != false ? this.totalDuration() : this.duration()) / this._timeScale
        )
    }
    ...
    var TweenLite = _class(
            ...
            function (target, duration, vars) {
                ...
                this.target = target =
                    typeof target !== 'string' ? target : TweenLite.selector(target) || target
                ...
                if (
                    (isSelector || target instanceof Array || (target.push && _isArray(target))) &&
                ...
                if (
                    ...
                    (duration === 0 && this._delay === 0 && this.vars.immediateRender !== false)
                ) {
    ...
    Animation._updateRoot = TweenLite.render = function () {
        ...
        _rootFramesTimeline.render(
            (_ticker.frame - _rootFramesTimeline._startTime) * _rootFramesTimeline._timeScale,
            false,
            false
        )
        if (_lazyTweens.length) {
        ...
        _applyOverwrite = function (target, tween, props, mode, siblings) {
            ...
            while (--i > -1) {
                if ((curTween = siblings[i]) === tween || curTween._gc || curTween._paused) {
                    ...
                    if (
                        curTween._startTime + curTween.totalDuration() / curTween._timeScale >
                        startTime
                    )
                        if (
                            !(
    ...
    p._initProps = function (target, propLookup, siblings, overwrittenProps, index) {
        ...
        for (p in this.vars) {
            ...
            if (_reservedProps[p]) {
                ...
                _plugins[p] &&
                (plugin = new _plugins[p]())._onInitTween(target, this.vars[p], this, index)
            ) {
                //t - target 		[object]
                //p - property 		[string]
                //s - start			[number]
        ...
        if (time >= duration - _tinyNum && time >= 0) {
            ...
            if (duration === 0)
                if (self._initted || !self.vars.lazy || force) {
                    ...
                    self._rawPrevTime = rawPrevTime =
                        !suppressEvents || time || prevRawPrevTime === time ? time : _tinyNum //when the playhead arrives at EXACTLY time 0 (right on top) of a zero-duration tween, we need to discern if events are suppressed so that when the playhead moves again (next time), it'll trigger the callback. If events are NOT suppressed, obviously the callback would be triggered in this render. Basically, the callback should fire either when the playhead ARRIVES or LEAVES this exact spot, not both. Imagine doing a timeline.seek(0) and there's a callback that sits at 0. Since events are suppressed on that seek() by default, nothing will fire, but when the playhead moves off of that position, the callback should fire. This behavior is what people intuitively expect. We set the _rawPrevTime to be a precise tiny number to indicate this scenario rather than using another property/variable which would increase memory usage. This technique is less readable, but more efficient.
                }
        } else if (time < _tinyNum) {
            //to work around occasional floating point math artifacts, round super small values to 0.
            self._totalTime = self._time = 0
            ...
            if (self._easeType) {
                ...
                self.ratio =
                    type === 1 ? 1 - r : type === 2 ? r : time / duration < 0.5 ? r / 2 : 1 - r / 2
            } else {
